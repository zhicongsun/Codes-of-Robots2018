...
int main(void)
{
  ...
  while (1)
  {	
	  	limitswtch=HAL_GPIO_ReadPin(GPIOF,GPIO_PIN_10);

	  //egg_task();
		//IMU_Get_Data();不能在while(1)里面获取陀螺仪值，这样会打断定时器里的获取，应放在定时器的6623控制中同步获取
    //var[0]=moto_chassis[0].speed_rpm;
    //var[1]=RC_CtrlData.rc.ch0;
...
#define CMD_WARE     3
    ...
    u8 cmdr[2] = {~CMD_WARE, CMD_WARE};    //串口调试 使用的后命令
		...
		HAL_UART_Transmit(&huart2,wareaddr,waresize,1000);
...
void all_bsp_init(void)
{
  ...
  MX_USART1_UART_Init();//遥控器，中断抢占优先级为0
	...
	/***************************************************************/
  MX_USART3_UART_Init();//裁判系统，中断抢占优先级为1
	HAL_UART_Receive_IT(&huart3, uart3_rx_buff, 1);//裁判系统
...
void all_pid_init(void)//PID参数初始化
{
	...
	PID_struct_init(&Cm3508_spd_pid[4], POSITION_PID, 7000,5000,P_cyl_spd,I_cyl_spd,D_cyl_spd);
//	PID_struct_init(&Cm3508_ang_pid[4], POSITION_PID, 3000,3000,P_cyl_ang,I_cyl_ang,D_cyl_ang);
	PID_struct_init(&Cm3508_spd_pid[5], POSITION_PID, 7000,5000,P_cyl_spd,I_cyl_spd,D_cyl_spd);
//	PID_struct_init(&Cm3508_ang_pid[5], POSITION_PID, 3000,3000,P_cyl_ang,I_cyl_ang,D_cyl_ang);
	
	PID_struct_init(&Follw_3508_pid, POSITION_PID, 1500,500,P_Follow,I_Follow,D_Follow);
	//20180406 pitch轴双环
	PID_struct_init(&Gm6623_pit_ang_pid, POSITION_PID, 300,300,P_6623_pit_ang,I_6623_pit_ang,D_6623_pit_ang);
	PID_struct_init(&Gm6623_pit_spd_pid, POSITION_PID, 5000,5000,P_6623_pit_spd,I_6623_pit_spd,D_6623_pit_spd);
	//YAW角度环PID参数20180405
	PID_struct_init(&Gm6623_yaw_ang_pid, POSITION_PID, 300,300,P_6623_yaw_ang,I_6623_yaw_ang,D_6623_yaw_ang);
	//随动yaw
		PID_struct_init(&Gm6623_yaw_spd_pid, POSITION_PID, 10000,10000,P_yaw_spd,I_yaw_spd,D_yaw_spd);
	//自动瞄准yaw
	  PID_struct_init(&autu_Gm6623_yaw_ang_pid, POSITION_PID, 10000,10000,P_autu_6623_yaw_ang,I_autu_6623_yaw_ang,D_autu_6623_yaw_ang);
