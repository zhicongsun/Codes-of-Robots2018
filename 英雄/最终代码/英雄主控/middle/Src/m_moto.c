...
#include "tim.h"
 ...
 u8 i;
moto_measure_t moto_chassis[6] = {0};//3508测量量
moto_measure_t moto_gym[2] = {0};//6623测量量
	...
	//ignore can1 or can2.
		switch(_hcan->pRxMsg->StdId)
		{
				...
				case CAN_2006Moto_ID:
						moto_toggle.msg_cnt++ <= 50	?	get_moto_offset(&moto_toggle, _hcan) : get_moto_measure(&moto_toggle, _hcan);
						break;
	  }
...
//	ptr->fited_angle = sum / FILTER_BUF_LEN;
	...
	if(ptr->angle - ptr->last_angle > 4096)
		...
		ptr->round_cnt ++;
	ptr->total_angle = ptr->round_cnt * 8192 + ptr->angle ;//- ptr->offset_angle;//用totalangle把-offset注释掉
