...
{  
    ...
    HAL_TIM_PWM_ConfigChannel(&TIM12_Handler,&TIM12_CH2Handler,TIM_CHANNEL_2);//配置TIM12通道1
    HAL_TIM_PWM_Start(&TIM12_Handler,TIM_CHANNEL_2);//开启PWM通道1
	  HAL_TIM_PWM_MspInit(&TIM12_Handler);
}
...
void HAL_TIM_Base_MspInit(TIM_HandleTypeDef* tim_baseHandle)
{
  ...
  /* USER CODE END TIM5_MspInit 0 */
    ...
    __HAL_RCC_TIM5_CLK_ENABLE();
		HAL_NVIC_SetPriority(TIM5_IRQn,4,3);    //设置中断优先级，抢占优先级1，子优先级3
  ...
  else if(tim_baseHandle->Instance==TIM6)
  {
    ...
    HAL_NVIC_SetPriority(TIM6_DAC_IRQn, 0, 1);//抢占优先级0，子优先级1
