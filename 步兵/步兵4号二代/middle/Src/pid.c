...
/*************  底盘运动  *********/
float P_pos_ang=0.6,I_pos_ang=0,D_pos_ang=10;
float P_3508_spd=5,I_3508_spd=0.0005,D_3508_spd=0;
float P_Follow=6,I_Follow=0.04,D_Follow=0.2;
float P_Follow_speed=6,I_Follow_speed=0.01,D_Follow_speed=0.2;
float P_Power=0.001,I_Power=0,D_Power=0;

/*************  云台yaw  *********/

float P_6623_yaw_pos=2,I_6623_yaw_pos=0.08,D_6623_yaw_pos=25;
//0.5		0			0
//28		0			0
float P_6623_yaw_ang=0.5,I_6623_yaw_ang=0,D_6623_yaw_ang=0;
float P_6623_yaw_spd=28,I_6623_yaw_spd=0,D_6623_yaw_spd=0;
...
//3			0.09	9
//5.4		0.09	7
float P_6623_pit_pos_up=3,I_6623_pit_pos_up=0.09,D_6623_pit_pos_up=9;
float P_6623_pit_pos_down=5.4,I_6623_pit_pos_down=0.09,D_6623_pit_pos_down=7;
//0.8		0.04	0
//20		0			0
float P_6623_pit_ang=0.76,I_6623_pit_ang=0.045,D_6623_pit_ang=0;
float P_6623_pit_spd=25,I_6623_pit_spd=0,D_6623_pit_spd=0;

float autu_P_6623_pit_ang=8,autu_I_6623_pit_ang=0,autu_D_6623_pit_ang=0;

/*************  拨弹  *********/

float P_tog_ang = 0.15, 	I_tog_ang = 0.005,D_tog_ang = 0.0;
float P_tog_spd = 4.5, 		I_tog_spd = 0.05,D_tog_spd = 0;

//float P_Follow=9,I_Follow=0,D_Follow=0;
//float P_3508_spd=7,I_3508_spd=0.05,D_3508_spd=3;
//float P_6623_yaw_ang=1,I_6623_yaw_ang=0,D_6623_yaw_ang=0;
//float P_6623_pit_ang=1,I_6623_pit_ang=0.00,D_6623_pit_ang=0;
//float P_Toggler_spd=0,I_Toggler_spd=0,D_Toggler_spd=0;
//float P_Power=0,I_Power=0,D_Power=0;
pid_t pid_yaw_pos;
pid_tt pid_pit_pos_down_up;
...
//tongge
pid_t Gm6623_pit_ang_pid_slow;
pid_t Gm6623_pit_ang_pid_quick;
pid_t Gm6623_yaw_ang_pid_slow;
pid_t Gm6623_yaw_ang_pid_quick;
...
pid_t Follw_3508_pid_speed;//随动的3508pid
...
float pid_pit_down_up_calc(pid_tt* pid, float get, float set)
{
	  pid->get = get;
    pid->set = set;
    pid->err[1] = set - get;	//set - measure  
    if (pid->max_err != 0 && ABS(pid->err[1]) >  pid->max_err  )
			return 0;
	  if (pid->deadband != 0 && ABS(pid->err[1]) < pid->deadband)
			return 0;
		
		if(pid->err[1]>0)
		{
				pid->pout = pid->p1 * pid->err[1];
        pid->iout += pid->i1 * pid->err[1];
        pid->dout = pid->d1 * (pid->err[1] - pid->err[0] );
		}
		else
		{
				pid->pout = pid->p0 * pid->err[1];
        pid->iout += pid->i0 * pid->err[1];
        pid->dout = pid->d0 * (pid->err[1] - pid->err[0] );
		}
		    abs_limit(&(pid->iout), pid->IntegralLimit);
        pid->pos_out = pid->pout + pid->iout + pid->dout;
        abs_limit(&(pid->pos_out), pid->MaxOutput);
