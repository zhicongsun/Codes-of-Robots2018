...
//	int time_threshhold=1200;
  ...
  while (1) 
  {	
		shootnum=toggle_heat_ctr(Infantry, ReadData.robot_level, ReadData.bullet_spd, ReadData.bullet_freq, ReadData.bullet_type,ReadData.shoot_heat17,ReadData.shoot_heat42);

		var[0]=moto_toggle.speed_rpm;
		var[1]=speedtest;
		var[2]=Power_pid.err[NOW];
		var[3]=Power_pid.pos_out;
		var[4]=km_nor_yaw_spd_fb;
		var[5]=Gm6623_yaw_spd_pid.pos_out;
    var[6]=km_nor_yaw_ang_fb;
...
void all_bsp_init(void)
{
  ...
  MX_USART1_UART_Init();//遥控器，中断抢占优先级为0
	...
	HAL_UART_Receive_IT(&huart2,uart2_rx_bytes,16);//与tk1通信时要开开

	/***************************************************************/
	MX_USART6_UART_Init();//裁判系统
  HAL_UART_Receive_IT(&huart6, uart6_rx_buff, sizeof(uart6_rx_buff));//裁判系统
...
void all_pid_init(void)//PID参数初始化
{
	for(int i=0; i<4; i++)
	{
		PID_struct_init(&Cm3508_ang_pid[i], POSITION_PID, 6000,5000,P_pos_ang,I_pos_ang,D_pos_ang);  //4 motos angular rate closeloop.
		PID_struct_init(&Cm3508_spd_pid[i], POSITION_PID, 5000,5000,P_3508_spd,I_3508_spd,D_3508_spd);  //4 motos angular rate closeloop.
	}
	PID_struct_init(&Follw_3508_pid, POSITION_PID, 5000,5000,P_Follow,I_Follow,D_Follow);//底盘与云台夹角PID，目标为中值
	PID_struct_init(&Follw_3508_pid_speed, POSITION_PID, 5000,5000,P_Follow_speed,I_Follow_speed,D_Follow_speed);//底盘与云台夹角速度PID
	//pitch
	PIDT_struct_init(&pid_pit_pos_down_up,POSITION_PID, 5000, 5000, 
																		P_6623_pit_pos_up, I_6623_pit_pos_up, D_6623_pit_pos_up,
																		P_6623_pit_pos_down, I_6623_pit_pos_down, D_6623_pit_pos_down);
	PID_struct_init(&Gm6623_pit_ang_pid, POSITION_PID, 2000,2500,P_6623_pit_ang,I_6623_pit_ang,D_6623_pit_ang);//双环
	PID_struct_init(&Gm6623_pit_spd_pid, POSITION_PID, 5000,5000,P_6623_pit_spd,I_6623_pit_spd,D_6623_pit_spd);
	//yaw
	PID_struct_init(&pid_yaw_pos, POSITION_PID, 5000,5000,P_6623_yaw_pos,I_6623_yaw_pos,D_6623_yaw_pos);//单位置环
	PID_struct_init(&Gm6623_yaw_ang_pid, POSITION_PID, 2000,1000,P_6623_yaw_ang,I_6623_yaw_ang,D_6623_yaw_ang);//双环
	PID_struct_init(&Gm6623_yaw_spd_pid, POSITION_PID, 5000,5000,P_6623_yaw_spd,I_6623_yaw_spd,D_6623_yaw_spd);
	//自动瞄准yaw
	PID_struct_init(&Gm6623_pit_ang_pid_slow, POSITION_PID, 300,300,0.25,0.01,0.02);
	PID_struct_init(&Gm6623_pit_ang_pid_quick, POSITION_PID, 300,300,0.3,0.007,1.0);
	PID_struct_init(&Gm6623_yaw_ang_pid_slow, POSITION_PID, 500,25,0.17,0,0.1);
	PID_struct_init(&Gm6623_yaw_ang_pid_quick, POSITION_PID, 500,25,5,0.007,1);

	PID_struct_init(&autu_Gm6623_yaw_ang_pid, POSITION_PID, 5000,5000,P_autu_6623_yaw_ang,I_autu_6623_yaw_ang,D_autu_6623_yaw_ang);
	...
	//功率环
	//PID_struct_init(&Power_pid, POSITION_PID, 4000,4000,P_Power,I_Power,D_Power);
